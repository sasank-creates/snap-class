...
import pandas as pd
from src.components.dialog_attendendance_results import attendance_result_dialog
...
def teacher_screen_register():                               # ✅ fixed capital S typo
    ...
    with c1:
        header_dashboard()
    with c2:
        if st.button("Go back to Home", type="secondary", key="registerbackbtn", shortcut="control+backspace"):  # ✅ unique key
            st.session_state['login_type']=None
            st.rerun()
    ...
    teacher_password = st.text_input("Enter password", placeholder="Enter your password here", type="password")
    teacher_confirm_password = st.text_input("Confirm password", placeholder="Confirm your password here", type="password")
